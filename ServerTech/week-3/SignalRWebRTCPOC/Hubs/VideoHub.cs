...
namespace SignalRWebRTCPOC.Hubs
{
    public class VideoHub : Hub<IConnectionHub>
    {
        ...
        public async Task CallUserVideo(User targetConnectionId, RPC offer)
        {
            ...
            await Clients.Client(targetConnectionId.ConnectionId).IncomingCall(callingUser, offer);
        ...
        public async Task AnswerVideo(bool acceptCall, User targetConnectionId, RPC Answer)
        {
            var callingUser = _Users.SingleOrDefault(u => u.ConnectionId == Context.ConnectionId);
            var targetUser = _Users.SingleOrDefault(u => u.ConnectionId == targetConnectionId.ConnectionId);
            ...
            await Clients.Client(targetConnectionId.ConnectionId).CallAccepted(callingUser, Answer);
